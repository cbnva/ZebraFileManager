...
{
    public class USBPrinter2 : Printer
    {
        ...
        static Dictionary<byte[], AutoResetEvent> requests = new Dictionary<byte[], AutoResetEvent>();
        ...
        public override byte[] RunCommand(byte[] command, bool response = true)
        {
            ...
            lock (commandLock)
            {
                using (var sh = UsbPrinterResolver.OpenUSBPrinterPort(PrinterName))
                {
                            ...
                            if (!t.Wait(timeout))
                            {
                                ...
                                if (ioThread != 0)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        r = UsbPrinterResolver.CancelSynchronousIo(tHandle);
                                    }
                                    finally
                                    {
                                        if (tHandle != IntPtr.Zero)
                                            UsbPrinterResolver.CloseHandle(tHandle);
                                    }
                                }
                                break;
                            }
        ...
        public static class UsbPrinterResolver
        {
            ...
            [DllImport("kernel32.dll", SetLastError = true)]
            internal static extern int CancelSynchronousIo(IntPtr hThread);
            ...
            public static string GetUSBPathFromPNPID(string PrinterName)
            {
            ...
            public static Microsoft.Win32.SafeHandles.SafeFileHandle OpenUSBPrinterPort(string PNPID)
            {
